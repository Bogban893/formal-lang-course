...
Обратите внимание, что реальная оценка за задачу может быть снижена, в том числе по результатам презентации.

**Обратите внимание, что подразумевается необходимость предварительно провести оптимизации ранее реализованных решений (если это требуется) и ставить эксперименты с максимально оптимизированной версией. Например, можно использовать более высокопроизводительные библиотеки линейной алгебры и т.д. Ставить эксперименты предлагается на относительно больших графах (указано ниже), при этом результат в духе "у меня 10 часов работало и не завершилось" означает только то, что у вас не самая аккуратная реализация. По крайней мере, если это происходит почти на всех алгоритмах. Но нужно иметь в виду, что одни алгоритмы существенно медленнее других.**
...
- [ ] Сформировать набор данных.
  - [ ] Выбрать некоторые графы из [набора](https://formallanguageconstrainedpathquerying.github.io/CFPQ_Data/graphs/index.html). **Выбирать графы с количеством вершин не менее 10 000 (десяти тысяч), хотя бы по одному из каждой категории, представленной в наборе (RDF, Java points-to и т.д.).** Не забудьте обосновать, почему выбрали именно эти графы.
...
**Обратите внимание, что подразумевается необходимость предварительно провести оптимизации ранее реализованных решений (если это требуется) и ставить эксперименты с максимально оптимизированной версией.  Например, можно использовать более высокопроизводительных библиотеки линейной алгебры и т.д. Ставить эксперименты предлагается на относительно больших графах (указано ниже), при этом результат в духе "у мен 10 часов работало и не завершилось" означает только то, что у вас не самая аккуратная реализация. По крайней мере, если это происходит почти на всех алгоритмах. Но нужно иметь ввиду, что одни алгоритмы существенно медленнее других.**
...
- [ ] Сформировать набор данных.
  - [ ] Выбрать некоторые графы из [набора](https://formallanguageconstrainedpathquerying.github.io/CFPQ_Data/graphs/index.html). **Выбирать графы с количеством вершин не менее 10 000 (десяти тысяч), хотя бы по одному из каждой категории, представленной в наборе (RDF, Java points-to и т.д.).** Не забудьте обосновать, почему выбрали именно эти графы. Обратите внимание, что в наборе есть графы и грамматики для различных прикладных задач (анализ RDF, анализ указателей в С, анализ Java-программ). Рекомендуется выбирать графы, относящиеся к различным областям.
